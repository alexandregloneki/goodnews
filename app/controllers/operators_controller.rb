class OperatorsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @operator.save
        format.html { redirect_to @operator, :flash => { :success =>'Operator was successfully created.'} }
        format.json { render json: @operator, status: :created, location: @operator }
      else
        flash.now[:error] = 'Operator was not created.'
  ...
  def update
    ...
    respond_to do |format|
      if @operator.update_attributes(params[:operator])
        format.html { redirect_to @operator, :flash=>{:success=> 'Operator was successfully updated.'} }
